...
```
src/
├─ app/            공통 · 랜딩페이지(LandingPage.tsx), 라우터(router.tsx), 홈(HomePage.tsx), 연금저축 관리(PensionSavingsPage.tsx)
...
`/`는 랜딩페이지(제출·데모 링크 공유용)이고, 실제 시연 화면은 전부 `/app` 아래에 있습니다.
아래 표의 주소는 `/app`을 붙여서 엽니다 — 예: `/home` → 실제 주소는 `/app/home`.
코드에서는 `/home`처럼 그대로 쓰면 됩니다(라우터가 `basename="/app"`으로 자동 처리).
